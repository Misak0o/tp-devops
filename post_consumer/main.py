...
def main(multiple, kafka_host):
    ...
    consumer = KafkaConsumer(
        ...
        auto_offset_reset="latest",  # start at earliest message
        enable_auto_commit=True,  # auto commit offsets
        group_id="super_mega_cool_group",  # consumer group
    ...
    try:
        ...
        for message in consumer:
            transformed_post = transform_post(message.value)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--multiple", action="store_true", help="Send multiple messages"
    )
    parser.add_argument(
        "--kafka_host",
        type=str,
        required=False,
        default=None,
        help="The Kafka host address, changing BigQuery target to Kafka",
    )
